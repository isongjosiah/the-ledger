...
export class RedisStreamQueue implements IMessagingQueue {
  ...
  constructor(streamKey: string, groupName: string) {
    ...
    this.client.connect().then(async () => {
      try {
        await this.client.xGroupCreate(this.streamKey, this.groupName, "$", {
          MKSTREAM: true,
        });
      } catch (err: any) {
        if (err.message.includes("BUSYGROUP")) {
          console.log(
            `[Redis]: consumer group ${this.groupName} already exists`,
          );
          return;
        }
        console.error(
          `[Redis]: failed to create consumer group ${this.groupName} => ${err}`,
        );
      }
    });
  }
  ...
  async consume(
    ...
    while (true) {
      try {
          ...
          for (const stream of response) {
            for (const message of stream.messages) {
              ...
              try {
                ...
                console.error(
                  `[Redis]: consumer ${consumerName} failed to process message ${id}`,
                );
              }
            }
          }
        ...
        console.error(
          `[Redis]: consumer ${consumerName} failed to consume from stream => ${err}`,
        );
      }
    }
  }
