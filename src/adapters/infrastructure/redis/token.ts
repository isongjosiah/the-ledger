...
export class RedisTokenRepository implements ITokenRepository {
  ...
  constructor() {
    ...
    this.client.connect().catch((err) => console.error(`[Redis]: connection failed: ${err}`)) //TODO: just crash the app if this doesn't conect
  }

  async setAccessToken(key: string, token:Token, expirationInSec = 3600): Promise<void>{
    await this.client.set(key, JSON.stringify(token.accessTokenJson(expirationInSec)), {EX: expirationInSec})
  }
  ...
  async getAccessToken(key: string): Promise<Token | null> {
    ...
    const tokenDetail = new Token(data.userId, data.userName, data.accessToken, data.refreshToken)
    return tokenDetail
  }
  ...
  async setRefreshToken(key: string, token:Token, expirationInSec = 3600): Promise<void>{
    await this.client.set(key, JSON.stringify(token.accessTokenJson(expirationInSec)), {EX: expirationInSec})
  }
