...
export class GeneralLedgerAccountRepository
  implements IGeneralLedgerAccountRepository
{
  async create(
    ...
    const createdDocs: IGeneralLedgerDocument[] =
  ...
  async findByCurrency(currency: string): Promise<GeneralLedger[]> {
    const docs = await generalLedgerModel
      .find({ currency: currency.toUpperCase() })
      .exec();
    return docs.map(
      (doc) =>
        new GeneralLedger(
  ...
  async update(
    ...
    const updatedDoc = await generalLedgerModel
      .findByIdAndUpdate(
        account.id,
        {
          accountName: account.accountName,
